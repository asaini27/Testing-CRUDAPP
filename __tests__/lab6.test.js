describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const button = await page.$('product-item');
    // Grab the shadowRoot of that element (it's a property), then query a button from that shadowRoot.
    const shadowRoot = await button.getProperty('shadowRoot');
    // Once you have the button, you can click it and check the innerText property of the button.
    const button2 = await shadowRoot.$('button');
    await button2.click();
    const buttontext = await button2.getProperty('innerText');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const buttons = await page.$$('product-item');
    // get the shadowRoot and query select the button inside, and click on it.
    for (let i = 0 ; i < buttons.length; i++){
      const shadowRoot = await buttons[i].getProperty('shadowRoot');
      const button = await shadowRoot.$('button');
      await button.click();
      console.log('Clicked button ${i+1}/${buttons.length}');
    }
    // Check to see if the innerText of #cart-count is 20
    const cartcount = await page.$eval('#cart-count', element => parseInt(element.innerText, 10));
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const value = await cartcount.getProperty('innerText');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const cartcount = await page.$('#cart-count');
    const cart = await cartcount.getProperty('innerText');
    const value = await cart.jsonValue();
    expect(value).toBe('0');
